...
function Pet() {
  ...
  useEffect(() => {
    ...
    fetch(variables.API_URL + '/Pets/PetShopList')
  ...
  const handleSubmit = (e) => {
    ...
    fetch(variables.API_URL + '/Support/CreateSupportToBuyPet', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      ...
      toastr.success('Başvuru İsteğiniz Gönderildi.');
    }
      handleCloseModal();
  };
  const feedback = () => {
    toastr.success('Başvurunuz Başarıyla Gönderildi.', 'Teşekkürler');
  };
  return (
    <div> 
           <Navbar />
    ...
    <div className="table-container" style={{ paddingTop: '50px', paddingLeft:'20px', paddingRight:'20px',paddingBottom:'20%' }}>
      ...
      <div className="alert" style={{  backgroundColor:'#88aa5b',color: 'black', textAlign: 'center', marginBottom: '20px' }}   >
            <h2 style={{ textAlign: 'center', fontFamily: 'Montserrat', fontSize: '36px', fontWeight: 'bold', color: 'white', letterSpacing: '2px' }}>Pet Shop Satılık Hayvan Listesi</h2>
          </div>
   ...
   <Table className="mt-4" striped bordered hover size="sm">
                    ...
                    <tbody>
                      ...
                      <button
                        onClick={() => {
                          if (userDetails.isPurchase === true) {
                        ...
                        style={{
                          backgroundColor:
                            x.petStatus.statusType === 'Satışta'
                              ? '#88aa5b'
                              : x.petStatus.statusType === 'İşleme Alındı'
                                ? '#F0E68C'
                                : '',
                          pointerEvents:
                            x.petStatus.statusType === 'İşleme Alındı' ? 'none' : 'auto',
                        }}
                      >
                        <a style={{ color: 'white' }}>
                          {x.petStatus.statusType === 'İşleme Alındı' ? 'Onay Bekliyor' : 'Arkadaş Edin'}
                        </a>
                      </button>
                    </td>
        ...
        <Modal.Header closeButton>
          <Modal.Title>Evcil Hayvan Satın Alma Başvuru Formu</Modal.Title>
        </Modal.Header>
        <Modal.Body>
